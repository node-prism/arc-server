...
export class ArcServer {
    ...
    static createServer(host, port, secure) {
        ...
        // authenticate
        this.duplex.command(0, async (payload, connection) => {
        ...
        // refresh tokens
        this.duplex.command(1, async (payload, connection) => {
        ...
        // query
        this.duplex.command(2, async (payload, connection) => {
            const { collection, operation, data, accessToken } = payload;
            if (!ValidateAccessToken(accessToken).valid)
                return { error: "Invalid access token" };
            return this.queryHandler.query(payload, connection);
        });
        // create user
        this.duplex.command(3, async (payload, connection) => {
        ...
        // remove user
        this.duplex.command(4, async (payload, connection) => {
            const { username, password, accessToken } = payload;
            if (!username || !password)
                return { error: "Invalid username or password" };
            if (!ValidateAccessToken(accessToken).valid)
...
export class CollectionManager {
    ...
    createCollectionWithOptions(name, options = {}) {
        if (this.collections[name]) {
